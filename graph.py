...
class TrafficNetwork(Graph):
    ...
    def __cast(self):
        ...
        if self.__origins != None and self.__destinations != None:
            ...
            self.__OD_pairs = self.__generate_OD_pairs1()
            self.__links = self.edges()
            self.__paths, self.__paths_category = self.generate_paths_by_demands()
            # LP Matrix = Link-Path Incidence Matrix
            self.__LP_matrix = self.generate_LP_matrix()
    
    def __generate_OD_pairs1(self):
    ...
    def OD_paths(self, paths_category):
        """Generate the list representing which paths contained in each OD pair
        """
        OD_paths = [[None]]*len(self.__generate_OD_pairs1())

        for i in range(len(OD_paths)):
            OD_paths[i] = []
        
        for i in range(len(paths_category)):
            OD_paths[paths_category[i]].append(i)
            
        return OD_paths
        

    def generate_paths_by_demands(self):
